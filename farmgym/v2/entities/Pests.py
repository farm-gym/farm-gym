...
class Pests(Entity_API):
    def __init__(self,field,parameters):
        ...
        self.variables = {}
        self.variables["plot_population#nb"] = fillarray(X,Y,(0,1000), 0.)#np.full((X,Y),fill_value=Range((0,1000),0.))
        ...
        weeds = [self.field.entities[e] for e in self.field.entities if checkissubclass(self.field.entities[e].__class__, 'Weeds')]
        self.variables["onplant_population#nb"] = {}
        for i in range(len(plants)):
            self.variables["onplant_population#nb"][plants[i].name] = fillarray(X,Y,(0,1000), 0.)#np.full((X,Y),fill_value=Range((0,1000),0.))
        for i in range(len(weeds)):
            self.variables["onplant_population#nb"][weeds[i].name] = fillarray(X,Y,(0,1000), 0.)#np.full((X,Y),fill_value=Range((0,1000),0.))
        ...
        #
        self.variables["total_cumulated_plot_population#nb"] = Range((0,10000),0.)

        self.dependencies = {'Weather','Soil','Plant','Birds'}

    def get_parameter_keys(self):
            return ['min_population#nb','max_population#nb','arrival_frequency#day-1','appear_conditions','leave_conditions']

    def reset(self):
        X = self.field.X
        Y = self.field.Y

        self.variables["plot_population#nb"] = fillarray(X, Y, (0, 1000),
                                                         0.)  # np.full((X,Y),fill_value=Range((0,1000),0.))

        plants = [self.field.entities[e] for e in self.field.entities if
                  checkissubclass(self.field.entities[e].__class__, 'Plant')]
        weeds = [self.field.entities[e] for e in self.field.entities if
                 checkissubclass(self.field.entities[e].__class__, 'Weeds')]
        self.variables["onplant_population#nb"] = {}
        for i in range(len(plants)):
            self.variables["onplant_population#nb"][plants[i].name] = fillarray(X, Y, (0, 1000),
                                                                                0.)  # np.full((X, Y), fill_value=Range((0, 1000), 0.))
        for i in range(len(weeds)):
            self.variables["onplant_population#nb"][weeds[i].name] = fillarray(X, Y, (0, 1000),
                                                                               0.)  # np.full((X,Y),fill_value=Range((0,1000),0.))
            ...
            for y in range(self.field.Y):
                ...
                for k in neighbors.keys():
                    ...
                    if (weather.variables["wind"]['direction'].value == k):
                        w[k] += weather.variables["wind"]['speed#km.h-1'].value/10.
                W = np.sum(list(w.values()))
                pop = self.variables["plot_population#nb"][x, y].value
                for k in neighbors.keys():
                    nbp = min(np.ceil(w[k]/W * pop), self.variables["plot_population#nb"][x,y].value)
                    self.variables["plot_population#nb"][neighbors[k]].set_value( self.variables["plot_population#nb"][neighbors[k]].value  + nbp)
                    self.variables["plot_population#nb"][x,y].set_value(self.variables["plot_population#nb"][x,y].value - nbp)
                ...
                if (is_arrival):
                    nb_edge_arrival = self.np_random.randint(self.parameters['min_population#nb'],self.parameters['max_population#nb'])
                else:
                    nb_edge_arrival =0

                p = self.parameters['leave_conditions']
                q = []
                maxd = max(field.X,field.Y)/2.
                q.append((p['sensitivity_dist_edge'], field.distance_to_edge((x, y)), maxd, np.infty))
                q.append((p['sensitivity_death_birds'],nb_birds_eating_pests,0,p['death_birds_max']))
                q.append((p['sensitivity_T'], weather.variables['air_temperature']['mean#°C'].value, p['T_min'],p['T_max']))
                q.append((p['sensitivity_pesticide'], soil.variables['amount_cide#g']['pests'][x, y].value, -np.infty, p['pesticide_min']))
                # TODO: Add total repulsive/attractive effect of plants on plot!
                q_stay = expglm(p['sensitivity_0'], q)

                self.variables["plot_population#nb"][x, y].set_value(self.np_random.binomial( (int) (self.variables["plot_population#nb"][x,y].value),q_stay,1)[0])

                p= self.parameters['appear_conditions']
                q = []
                q.append((p['sensitivity_dist_edge'], field.distance_to_edge((x,y)), -np.infty, 0.))
                q.append((p['sensitivity_T'], weather.variables['air_temperature']['mean#°C'].value, p['T_min'],p['T_max']))
                q.append((p['sensitivity_pesticide'], soil.variables['amount_cide#g']['pests'][x,y].value, -np.infty, p['pesticide_min']))
                # TODO: Add total repulsive/attractive effect of plants on plot!
                ...
                new_value= min( self.variables["plot_population#nb"][x, y].value + nb_appear, self.parameters['max_population#nb'])
                self.variables["total_cumulated_plot_population#nb"].set_value(self.variables["total_cumulated_plot_population#nb"].value  + new_value-self.variables["plot_population#nb"][x, y].value)
            ...
            for y in range(self.field.Y):
                ...
                for i in range(len(plants)):
                    if (plants[i].is_active((x,y))):
                        weights[i]=np.exp(-plants[i].parameters['pest_repulsive_effect#float'])*plants[i].variables['population#nb'][x,y].value
                for i in range(len(weeds)):
                    if (weeds[i].variables["grow#nb"][x,y].value+weeds[i].variables["flowers#nb"][x,y].value>0):
                        weights[i+len(plants)]=np.exp(-weeds[i].parameters['pest_repulsive_effect#float'])*(weeds[i].variables['grow#nb'][x,y].value+
                                                                                                            weeds[                                                                                                               i].variables[
                                                                                                                'flowers#nb'][
                                                                                                                x, y].value)
                if(np.sum(weights)>0):
