...
from farmgym.v2.entity_api import Entity_API, Range, fillarray, checkissubclass, expglm, expglmnoisy
import logging
import numpy as np
...
class Plant(Entity_API):
    ...
    def reset(self):
        ...
        self.variables["stage"] = fillarray(X, Y, self.stages, "none")
        self.variables["global_stage"] = Range(self.stages + ["undefined"], "none")
        self.variables["population#nb"] = fillarray(X, Y, (0, 100), 0.0)  # np.full((X,Y),fill_value=Range((0,100),0.))
        self.variables["size#cm"] = fillarray(X, Y, (0, 1000), 0.0)  # np.full((X,Y),fill_value=Range((0,1000),0.))
        self.variables["flowers_per_plant#nb"] = fillarray(
            X, Y, (0, 1000), 0.0
        )  # np.full((X,Y),fill_value=Range((0,1000),0.))
        self.variables["pollinator_visits#nb"] = fillarray(
            X, Y, (0, 1000), 0.0
        )  # np.full((X,Y),fill_value=Range((0,1000),0.))
        self.variables["flowers_pollinated_per_plant#nb"] = fillarray(
            X, Y, (0, 1000), 0.0
        )  # np.full((X,Y),fill_value=Range((0,1000),0.))
        self.variables["fruits_per_plant#nb"] = fillarray(X, Y, (0, 1000), 0.0)  # np.full((X,Y),fill_value=Range((0,1000),0.))
        self.variables["fruit_weight#g"] = fillarray(X, Y, (0, 100000), 0.0)  # np.full((X,Y),fill_value=Range((0,100000),0.))
        self.variables["harvest_weight#kg"] = Range((0, 1000000), 0.0)

        self.variables["age_seed#day"] = fillarray(X, Y, (0, 100), 0.0)  # np.full((X,Y),fill_value=Range((0,100),0.))
        self.variables["consecutive_nogrow#day"] = fillarray(
            X, Y, (0, 100), 0.0
        )  # np.full((X,Y),fill_value=Range((0,100),0.))
        self.variables["age_bloom#day"] = fillarray(X, Y, (0, 100), 0.0)  # np.full((X,Y),fill_value=Range((0,100),0.))
        self.variables["consecutive_noweight#day"] = fillarray(
            X, Y, (0, 100), 0.0
        )  # np.full((X,Y),fill_value=Range((0,100),0.))
        self.variables["age_ripe#day"] = fillarray(X, Y, (0, 100), 0.0)  # np.full((X,Y),fill_value=Range((0,100),0.))

        self.variables["cumulated_nutrients_N#g"] = fillarray(
            X, Y, (0, 10000), 0.0
        )  # np.full((X,Y),fill_value=Range((0,10000),0.))
        self.variables["cumulated_nutrients_P#g"] = fillarray(
            X, Y, (0, 10000), 0.0
        )  # np.full((X,Y),fill_value=Range((0,10000),0.))
        self.variables["cumulated_nutrients_K#g"] = fillarray(
            X, Y, (0, 10000), 0.0
        )  # np.full((X,Y),fill_value=Range((0,10000),0.))
        self.variables["cumulated_nutrients_C#g"] = fillarray(
            X, Y, (0, 10000), 0.0
        )  # np.full((X,Y),fill_value=Range((0,10000),0.))
        ...
        self.variables["cumulated_water#L"] = fillarray(X, Y, (0, 10000), 0.0)  # np.full((X,Y),fill_value=Range((0,10000),0.))
        self.variables["cumulated_stress_water#L"] = fillarray(
            X, Y, (0, 10000), 0.0
        )  # np.full((X,Y),fill_value=Range((0,10000),0.))

        self.variables["grow_size_threshold#cm"] = fillarray(
            X, Y, (0, 10000), 0.0
        )  # np.full((X,Y),fill_value=Range((0,10000),0.))
        self.variables["fruit_weight_threshold#g"] = fillarray(
            X, Y, (0, 100000), 0.0
        )  # np.full((X,Y),fill_value=Range((0,100000),0.))
        ...
        if self.variables["stage"][position].value in ["dead"]:
            for n in ["N", "K", "P", "C"]:
                ...
                self.variables["cumulated_nutrients_" + n + "#g"][position].set_value(
                    max(
                        0,
                        self.variables["cumulated_nutrients_" + n + "#g"][position].value - r[n],
                    )
                )
        return r
