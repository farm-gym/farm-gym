...
class Soil(Entity_API):
    def __init__(self,field,parameters):
        ...
        self.variables['available_N#g'] = fillarray(X,Y,(0,10000), 0.)#np.full((X,Y),fill_value=Range((0,10000),0.))
        self.variables['available_P#g'] = fillarray(X,Y,(0,10000), 0.)#np.full((X,Y),fill_value=Range((0,10000),0.))
        self.variables['available_K#g'] = fillarray(X,Y,(0,10000), 0.)#np.full((X,Y),fill_value=Range((0,10000),0.))
        self.variables['available_C#g'] = fillarray(X,Y,(0,10000), 0.)#np.full((X,Y),fill_value=Range((0,10000),0.))
        self.variables['available_Water#L']= fillarray(X,Y,(0,10000), 0.)#np.full((X,Y),fill_value=Range((0,10000),0.))

        self.variables['wet_surface#m2.day-1'] = fillarray(X,Y,(0,1000), 0.)#np.full((X,Y),fill_value=Range((0,1000),0.))

        self.variables['microlife_health_index#%'] = fillarray(X,Y,(0,100), 0.)#np.full((X,Y),fill_value=Range((0,100),0.))

        self.variables['amount_cide#g'] = {'pollinators': fillarray(X,Y,(0,10000), 0.),#np.full((X,Y),fill_value=Range((0,10000),0.)),
                                           'pests': fillarray(X,Y,(0,10000), 0.),#np.full((X,Y),fill_value=Range((0,10000),0.)),
                                           'soil': fillarray(X,Y,(0,10000), 0.),#np.full((X,Y),fill_value=Range((0,10000),0.)),
                                           'weeds': fillarray(X,Y,(0,10000), 0.)#np.full((X,Y),fill_value=Range((0,10000),0.))
                                           }


        #
        self.variables["total_cumulated_added_water#L"] = Range((0,100000),0.)
        self.variables['total_cumulated_added_cide#g'] = {'pollinators': Range((0,100000),0.),
                                                          'pests': Range((0,100000),0.),
                                                          'soil': Range((0,100000),0.),
                                                          'weeds': Range((0,100000),0.)}

        #Actions
        self.actions = {'water_discrete': {'plot' : field.plots, 'amount#L': list(np.linspace(1,15,15)), 'duration#min': [30,60]},
                        'water_continuous':{'plot' : field.plots, 'amount#L': (0.,20.), 'duration#min': [30,60]},                        }

        #Dependencies
        self.dependencies = {'Plant','Weather'}

    def get_parameter_keys(self):
            return ['max_water_capacity#L.m-3','depth#m','wilting_point#L.m-3','water_surface_absorption_speed#m2.day-1', 'bedrocks_release_N#mg.day-1','bedrocks_release_K#mg.day-1','bedrocks_release_P#mg.day-1','bedrocks_release_C#mg.day-1']
    ...
    def reset(self):
        ...
        for x in range(self.field.X):
            for y in range(self.field.Y):
                ...
                self.variables['available_Water#L'][x,y].set_value((0.5+0.5*self.np_random.rand())*self.parameters['max_water_capacity#L.m-3'] * self.field.plotsurface*self.parameters['depth#m'])
        ...
        self.variables["total_cumulated_added_water#L"].set_value(0.)
        #self.variables["total_cumulated_added_pesticide#g"] = 0.
        #self.variables["total_cumulated_added_herbicide#g"] = 0.
        self.variables['total_cumulated_added_cide#g']['pollinators'].set_value(0.)
        self.variables['total_cumulated_added_cide#g']['pests'].set_value(0.)
        self.variables['total_cumulated_added_cide#g']['soil'].set_value(0.)
        self.variables['total_cumulated_added_cide#g']['weeds'].set_value(0.)
        self.initialize_variables(self.initial_conditions)


    def update_variables(self,field,entities):
        max_water_plot_capacity=self.parameters['max_water_capacity#L.m-3']*self.field.plotsurface*self.parameters['depth#m']
        ...
        for x in range(self.field.X):
            for y in range(self.field.Y):
                ...
                if (weather.variables["rain_amount"].value=="Light"):
                    water_after_input=self.variables['available_Water#L'][x, y].value + 0.5*self.np_random.rand()*max_water_plot_capacity
                ...
                for n in ['N', 'K', 'P', 'C']:
                        self.variables['available_'+n+'#g'][x,y].set_value(self.variables['available_'+n+'#g'][x,y].value+ self.variables['microlife_health_index#%'][x,y].value/100.*self.parameters['bedrocks_release_'+n+'#mg.day-1']/1000.)
                ...
                for f in fertilizers:
                    ...
                    for n in ['N', 'K', 'P', 'C']:
                        self.variables['available_'+n+'#g'][x,y].set_value( self.variables['available_'+n+'#g'][x,y].value +release[n])
                ...
                for c in cides:
                    ...
                    for n in ['pollinators','pests','soil','weeds']:
                        self.variables['amount_cide#g'][n][x,y].set_value(self.variables['amount_cide#g'][n][x,y].value+ release*1000 * c.parameters[n])
                ...
                for w in weeds:
                    ...
                    for n in ['N', 'K', 'P', 'C']:
                        self.variables['available_'+n+'#g'][x,y].set_value( max(0.,self.variables['available_'+n+'#g'][x,y].value-  requirements[n+'#g']))

                    self.variables['available_Water#L'][x, y].set_value( max(0.,self.variables['available_Water#L'][x, y].value -  requirements['Water#L']))

                #Plant nutrients comsumption or release:
                milife = self.variables['microlife_health_index#%'][x, y].value/100.
                for p in plants:
                    ...
                    stress = {}
                    for n in ['N', 'K', 'P', 'C']:
                        v[n+'#g'] = min(self.variables['available_'+n+'#g'][x,y].value,milife*requirements[n])
                        stress[n+'#g'] = requirements[n]- v[n+'#g']
                        self.variables['available_'+n+'#g'][x,y].set_value( self.variables['available_'+n+'#g'][x,y].value- v[n+'#g']+release[n])
                    ...
                    w = min(requirement_water,max(self.variables['available_Water#L'][x,y].value-wp,0))
                ...
                water_evaporation_threshold= self.parameters['max_water_capacity#L.m-3'] * self.field.plotsurface * (self.parameters['depth#m']-self.parameters['total_evaporable_water#mm'] / 1000)
                soil_evaporated_water= min(soil_evaporated_water, max(self.variables['available_Water#L'][x,y].value-water_evaporation_threshold,0))
                ...
                self.variables['available_Water#L'][x,y].set_value( max(0,self.variables['available_Water#L'][x,y].value  - soil_evaporated_water))
                self.variables['wet_surface#m2.day-1'][x,y].set_value( max(self.variables['wet_surface#m2.day-1'][x,y].value-self.parameters['water_surface_absorption_speed#m2.day-1'],0))
                ...
                q = []
                q.append((1., self.variables['amount_cide#g']['soil'][x,y].value/100, 0, 0))
                q.append((0.5, water_surplus/max_water_plot_capacity, 0, 0))
                p_stayalive = expglm(0., q)
                ...
                self.variables['microlife_health_index#%'][x, y].set_value(((p_stayalive*(1+0.1*p_stayalive)+(1-p_stayalive)*(p_stayalive))*self.variables['microlife_health_index#%'][x, y].value))
                ...
                if (rain_intensity>0 or water_surplus>0):
                    ...
                    for n in ['N', 'K', 'P', 'C']:
                        self.variables['available_'+n+'#g'][x,y].set_value( max(0,self.variables['available_'+n+'#g'][x,y].value  -(rain_intensity*surf+water_surplus/max_water_plot_capacity)*(1-milife) ))
                    for n in ['pollinators','pests','soil','weeds']:
                        self.variables['amount_cide#g'][n][x,y].set_value( max(0, self.variables['amount_cide#g'][n][x,y].value*np.exp(- (rain_intensity*surf+water_surplus/max_water_plot_capacity))))
    ...
    def ground_evaporation(self, position, weather, plants, weeds, field):
        ...
        shadow_proportion = min((plantshadow + weedshadow) / self.field.plotsurface, 1.)
        wet_proportion= self.variables['wet_surface#m2.day-1'][position].value/self.field.plotsurface
        evapo_prop = min(1.-shadow_proportion,wet_proportion)

        #TODO: Multiply by coeff to stop/slow-down evaporation for clay/sand/loam.

        drop_proportion = (1.1-self.variables['microlife_health_index#%'][position].value/100)*self.field.plotsurface*self.parameters['depth#m']*self.parameters['water_leakage_max#L.m-3.day-1']*1000
    ...
    def act_on_variables(self,action_name,action_params):
        ...
        if (action_name == 'water_discrete' or 'water_continuous'):
                ...
                self.variables['wet_surface#m2.day-1'][x,y].set_value( self.variables['wet_surface#m2.day-1'][x,y].value +  self.field.plotsurface * (action_params['duration#min']/60.)/24. )# watering for 30minutes over 24h.
                self.variables["total_cumulated_added_water#L"].set_value(self.variables["total_cumulated_added_water#L"].value + (new_value- self.variables['available_Water#L'][x, y].value))
                self.variables['available_Water#L'][x, y].set_value( new_value )
                if (water_surplus>0):
                    milife = self.variables['microlife_health_index#%'][x, y].value/100.
                    for n in ['N', 'K', 'P', 'C']:
                        self.variables['available_' + n + '#g'][x, y].set_value(max(0,  self.variables['available_' + n + '#g'][
                                                                                        x, y].value -   water_surplus * (
                                                                                                1 - milife)))
                    for n in ['pollinators', 'pests', 'soil', 'weeds']:
                        self.variables['amount_cide#g'][n][x, y].set_value(max(0, self.variables['amount_cide#g'][n][x, y].value*np.exp( - water_surplus/max_water_plot_capacity)))

                    q = []
                    q.append((2., self.variables['amount_cide#g']['soil'][x, y].value, 0, 0))
                    q.append((0.5, water_surplus / max_water_plot_capacity, 0, 0))
                    p_stayalive = expglm(0., q)
                    self.variables['microlife_health_index#%'][x, y].set_value(((p_stayalive * (
                                1 + 0.1 * p_stayalive) + (1 - p_stayalive) * (p_stayalive)) * self.variables[
