...
class Fertilizer(Entity_API):
    def __init__(self,field,parameters):
        ...
        self.variables = {}
        self.variables['amount#kg'] = fillarray(X,Y,(0,1000), 0.)#np.full((X,Y),fill_value=Range((0,1000),0.))


        #
        self.variables["total_cumulated_scattered_amount#kg"] = Range((0,10000),0.)

        #Actions
        self.actions = {'scatter': {'plot': field.plots, 'amount#kg':(0,10)},
    ...
    def release_nutrients(self, position,soil):
        ...
        amount = self.variables['amount#kg'][position].value*self.parameters['base_absorption_speed#kg.week-1']/7.*(soil.variables['microlife_health_index#%'][position].value/100.)
        for n in ['N', 'K', 'P', 'C']:
    ...
    def act_on_variables(self,action_name,action_params):
        ...
        if (action_name == 'scatter'):
            ...
            self.variables["total_cumulated_scattered_amount#kg"].set_value(self.variables["total_cumulated_scattered_amount#kg"].value + action_params['amount#kg'])

        elif (action_name == 'scatter_bag'):
            x, y = action_params['plot']
            self.variables['amount#kg'][x, y].set_value(self.variables['amount#kg'][x, y].value  + action_params['amount#bag'] * self.parameters['bag#kg'])
            self.variables["total_cumulated_scattered_amount#kg"].set_value(self.variables["total_cumulated_scattered_amount#kg"].value + action_params['amount#bag'] * self.parameters['bag#kg'])
