...
def puissance_solaire(latitude, jour): # en W/m^2
       ...
       # Calcul de la puissance solaire
        H = H0 * (1 + 0.034 * math.cos(2 * math.pi * J / 365)) * math.cos(L) * math.cos(delta) * math.sin(h_s)
        return H
...
if __name__ == "__main__":
    ...
    for jour in [81,81+365//4,81+365//2,81+3*365//4]:
        for latitude in [-70,-66,-60,-50,-40,-30,-5,5,30,40,50,60,66,70,80]:
            puissance = puissance_solaire(latitude, jour)
            print(f"La puissance solaire reçue à la latitude {latitude} le jour {jour} est de {puissance} W/m².")

            irradiance = irradiance_perday(latitude, jour)
            print(f"L'irradiance solaire à la latitude {latitude} le jour {jour} est de {irradiance} kWh/m2 par jour soit {irradiance/0.626} mm/m2 potentiellement evapore par jour" )
