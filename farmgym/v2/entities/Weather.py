...
class Weather(Entity_API):
    ...
    def __init__(self,field,parameters):
        ...
        self.variables["air_temperature"] = {"mean#°C":Range((-100,100),0.),"min#°C":Range((-100,100),0.),"max#°C":Range((-100,100),0.)}
        self.variables["humidity_index#%"] = Range((0.,100.),0.)
        self.variables["wind"]={"speed#km.h-1":Range((0.,500),0.),"direction":Range(self.wind_directions,"W")}
        self.variables["sun_exposure#int5"] = Range(list(range(5)),0)
        self.variables["rain_amount"] = Range(self.rain_amounts,'None')
        self.variables["rain_intensity"] = Range((0.,1.),0.)

        self.variables['consecutive_frost#day'] = Range((0,10000),0.)
        self.variables['consecutive_dry#day'] = Range((0,10000),0.)

        self.variables["air_temperature.forecast"] = {"mean#°C":np.full(self.parameters['forecast_lookahead'],fill_value=Range((-100,100),0.)),
                                                      "min#°C":np.full(self.parameters['forecast_lookahead'],fill_value=Range((-100,100),0.)),
                                                      "max#°C":np.full(self.parameters['forecast_lookahead'],fill_value=Range((-100,100),0.))}
    ...
    def get_parameter_keys(self):
            return ['one_year_data_filename','air_temperature_noise','humidity_index_noise','rain_lightheavy_proba','rain_leakageintensity_light#%','rain_leakageintensity_heavy#%', 'forecast_lookahead','forecast_noise']
    ...
    def update_variables(self,field,entities):
        ...
        self.variables["humidity_index#%"].set_value(self.year_weather['RH'][day %365] + self.np_random.normal(0,self.parameters['humidity_index_noise'],1)[0])
        ...
        if (is_rain):
            ...
            self.variables["rain_amount"].set_value("Light" if self.np_random.rand()<=self.parameters['rain_lightheavy_proba'] else "Heavy")
            self.variables["rain_intensity"].set_value(self.parameters['rain_leakageintensity_light#%'] if self.variables["rain_amount"].value == "Light" else self.parameters['rain_leakageintensity_heavy#%'])
        else:
            ...
            self.variables["rain_intensity"].set_value(0.)

        self.variables["air_temperature.forecast"]["mean#°C"] = np.full(self.parameters['forecast_lookahead'],fill_value=Range((-100,100),0.))
        self.variables["air_temperature.forecast"]["min#°C"] = np.full(self.parameters['forecast_lookahead'],fill_value=Range((-100,100),0.))
        self.variables["air_temperature.forecast"]["max#°C"] = np.full(self.parameters['forecast_lookahead'],fill_value=Range((-100,100),0.))
        for i in range(self.parameters['forecast_lookahead']):
             ...
             self.variables["air_temperature.forecast"]["mean#°C"][i].set_value((self.year_weather['T'][(day+i) %365] + eps))
             self.variables["air_temperature.forecast"]["min#°C"][i].set_value((self.year_weather['Tmin'][(day+i) %365] + eps))
             self.variables["air_temperature.forecast"]["max#°C"][i].set_value((self.year_weather['Tmax'][(day+i) %365] + eps))
    ...
    def evapo_coefficient(self,field): # in mL/m2
        ...
        RA = ((24 * 60) / np.pi) * Gsc * dr * (
                ws * np.sin(np.pi / 180 * phi) * np.sin(delta) + np.cos(ws) * np.cos(
            np.pi / 180 * phi) * np.cos(
            delta))
        ...
        return np.maximum(0, 0.018 * ((1 - rh / 100) ** (0.2)) * (np.maximum(0, (tmax - tmin)) ** (0.3)) * (
                RA * np.sqrt(np.maximum(0, t_av + 10)) - 40) + 0.1 * (t_av + 20) * (1 - rh / 100) * (
                                  (u / 2) ** 0.6))
