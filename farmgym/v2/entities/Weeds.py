...
class Weeds(Entity_API):
    stages = ['none','seed','entered_grow','grow','entered_bloom','bloom','entered_fruit', 'fruit','entered_ripe','ripe','dead']
    def __init__(self,field,parameters):
        ...
        self.variables = {}
        self.variables["grow#nb"] = fillarray(X,Y,(0,1000), 0.)#np.full((X,Y),fill_value=Range((0,1000),0.))
        self.variables["seeds#nb"] = fillarray(X,Y,(0,1000), 0.)#np.full((X,Y),fill_value=Range((0,1000),0.))
        self.variables["flowers#nb"] = fillarray(X,Y,(0,1000), 0.)#np.full((X,Y),fill_value=Range((0,1000),0.))
    ...
    def get_parameter_keys(self):
            return ['appear_conditions','grow_conditions','max_new_seeds#nb','flowers_per_plant#nb']
    ...
    def update_variables(self,field,entities):
        ...
        p = 1./(0.+self.parameters['time_to_grow#day'])
        # Pick some positions:
        pos_seed = []
        pos_grow = []
        pos_bloom = []
      ...
      #  index_seed =(int) (np.floor(i_[0]*index))
       # index_grow =min(index_seed + (int) (np.ceil(i_[1]*index)), index)
        ...
        for pos in pos_seed: # Flowers to Seed
            ...
            z = self.np_random.binomial(self.variables["flowers#nb"][x, y].value, self.parameters['flower_to_seed#%'], 1)[0]
        ...
        for pos in pos_grow: # Seed to Grow
            ...
            q = []
            q.append(
                (p['sensitivity_grow_T'], weather.variables['air_temperature']['mean#°C'].value, p['grow_T_min'], p['grow_T_max']))
            q.append(
                (p['sensitivity_grow_RH'], weather.variables['humidity_index#%'].value, p['grow_RH_min'],
                 p['grow_RH_max']))
            q.append((p['sensitivity_grow_herbicide'], soil.variables['amount_cide#g']['weeds'][x, y].value, -np.infty,
...
#                (np.infty, soil.variables['available_C#g'][(x, y)].value, p['C_grow_consumption#g.mm-1'], np.infty))
            ...
            self.variables["total_cumulated_plot_population#nb"].set_value(self.variables["total_cumulated_plot_population#nb"].value + nb_sprouts)
        ...
        for pos in pos_bloom: # Grow to Flowers
            ...
            q.append((p['sensitivity_grow_herbicide'], soil.variables['amount_cide#g']['weeds'][x, y].value, -np.infty,
                      p['grow_herbicide_max#g']))
            p_flowers = expglm(self.parameters['sensitivity_flowers_0'], q) # np.exp(-self.parameters['sensitivity_flowers_0'])
            ...
            self.variables["flowers#nb"][x, y].set_value(self.variables["flowers#nb"][x, y].value + z*self.np_random.binomial(self.parameters['flowers_per_plant#nb'], p_flowers, 1)[0])
        ...
        for pos in positions:
            ...
            w = ((1. - draught) * soil.parameters['wilting_point#L.m-3'] + draught * soil.parameters[
                'max_water_capacity#L.m-3'])*soil.parameters['depth#m']*self.field.plotsurface
            q = []
            q.append((p['sensitivity_death_herbicide'], soil.variables['amount_cide#g']['weeds'][x, y].value, -np.infty,
                      p['death_herbicide_max#g']))
            q.append(
                (p['sensitivity_death_N'], soil.variables['available_N#g'][(x, y)].value, p['N_grow_consumption#g.mm-1'], np.infty))
            q.append(
                (p['sensitivity_death_K'], soil.variables['available_K#g'][(x, y)].value, p['K_grow_consumption#g.mm-1'], np.infty))
            q.append(
                (p['sensitivity_death_P'], soil.variables['available_P#g'][(x, y)].value, p['P_grow_consumption#g.mm-1'], np.infty))
            q.append(
                (p['sensitivity_death_C'], soil.variables['available_C#g'][(x, y)].value, p['C_grow_consumption#g.mm-1'], np.infty))
            q.append((p['sensitivity_death_Water'], soil.variables['available_Water#L'][(x, y)].value, w, np.infty))
    ...
    def requirement(self, position):
        ...
        p = self.parameters['grow_conditions']
        return {'N#g':nb*p['N_grow_consumption#g.mm-1'],'K#g':nb*p['N_grow_consumption#g.mm-1'],'P#g':nb*p['N_grow_consumption#g.mm-1'],'C#g':nb*p['N_grow_consumption#g.mm-1'],'Water#L':nb*p['Water_grow_consumption#mL.mm-1']*0.001}
    ...
    def compute_shadowsurface(self,position):
        ...
        r=self.parameters['size#cm']*0.01/2.
