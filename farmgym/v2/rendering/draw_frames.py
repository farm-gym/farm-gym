...
def one_frame(plant):
    ...
    for x in range(X):
        for y in range(Y):
            image.paste(images[plant.variables['stage'][x,y]], (im_width * y, im_height * x), mask=images[plant.variables['stage'][x,y]])
...
def dashboard(stage_growth, nb_of_day, q_level, day, name_plant='corn'):
    ...
    im_soil = Image.open(CURRENT_DIR / "sprites/soil.png")
    dashboard_picture = Image.new('RGB', (im_soil.width * wide + im_soil.width * 9,
    ...
    d.text((im_soil.width * (wide // 2), im_soil.height // 2), 'Day: {}'.format(day), font=font, fill='yellow')
