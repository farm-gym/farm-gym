...
class Entity_API():
    ...
    def assert_action_(self,action_key,action_value):
        ...
        if type(self.actions[action_key]) is tuple:
            assert self.actions[action_key][0] <= action_value <= self.actions[action_key][1], "Action value "+ action_value+ " for key " + action_key + " not in range ["+str(self.actions[action_key][0])+", "+ str(self.actions[action_key][1])+"]!"
        else:
        ...
        for p in action_params:
            ...
            if type(self.actions[action_name][p]) is tuple:
                ...
                assert self.actions[action_name][p][0] <= action_params[p] <= self.actions[action_name][p][1], \
                    "Action value " + str(action_params[p]) + " for key " + p + " not in range [" + str(
                        self.actions[action_name][p][0]) + ", " + str(self.actions[action_name][p][1]) + "]!"
            else:
                #print("ASSERT", action_name, self.actions[action_name], action_params, action_params[p], self.actions[action_name][p])
                if(p=='plot'):
                   assert str(action_params[p]) in self.actions[action_name][p], "PLOT" + str(action_params[p]) +" not in " + str(self.actions[action_name][p])
                else:
                    assert action_params[p] in self.actions[action_name][p], str(action_params[p]) +" not in " + str(self.actions[action_name][p])
            ...
            if ('sprites'in self.parameters):
                for key in self.parameters['sprites']:
                    self.images[key] = Image.open(
                        CURRENT_DIR / ("specifications/sprites/" + self.parameters['sprites'][key]))
...
if __name__ == "__main__":
    ...
    # print(y)
    z = np.full((2, 3), fill_value=Range((0, 5), 2.))
    print(z[0, 1].value)
    print(z[1, 1].value)
