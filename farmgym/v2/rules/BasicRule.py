...
class BasicRule(Rules_API):
    def __init__(self,init_configuration,free_observations,actions_configuration,terminal_CNF_conditions, max_action_schedule_cost=np.infty,initial_conditions_values=None):
        Rules_API.__init__(self, init_configuration,free_observations,actions_configuration,terminal_CNF_conditions,initial_conditions_values=initial_conditions_values)
    ...
    def filter_actions(self,farm,actions,is_observation_time):
         ...
         for a in actions:
            if self.is_allowed_action(a,is_observation_time):
                ...
                print("[Farmgym:Rules] Action", str(a), "is not allowed by configured rules. Note that this is ", "observation" if is_observation_time else "intervention","time.")
         ...
         if (farm.is_new_day):
             ...
             for observation_item in actions_schedule:
                 ...
                 cost = farm.scoring.observation_cost(farm.farmers[fa_key], farm.fields[fi_key], fi_key, entity,
                                                      variable_key, path)
