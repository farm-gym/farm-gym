...
def env():
    ...
    var = []
    var.append(("Field-0", "Weather-0", "air_temperature", lambda x:x["min#°C"].value, "Min air temperature (°C)", 'range_auto'))
    var.append(("Field-0", "Weather-0", "air_temperature", lambda x:x["mean#°C"].value, "Mean air temperature (°C)", 'range_auto'))
    var.append(("Field-0", "Weather-0", "air_temperature", lambda x:x["max#°C"].value, "Max air temperature (°C)", 'range_auto'))
    var.append(("Field-0", "Weather-0", "rain_amount", lambda x:x.gym_value(), "Rain amount", 'range_auto'))
    var.append(("Field-0", "Weather-0", "rain_intensity", lambda x:x.value, "Rain intensity", 'range_auto'))
    var.append(("Field-0", "Weather-0", "wind", lambda x:x["speed#km.h-1"].value, "Wind speed (km.h-1)", 'range_auto'))
    var.append(("Field-0", "Pests-0", "plot_population#nb", lambda x:x[(0,0)].value, "Insects (nb)", 'range_auto'))
    var.append(("Field-0", "Plant-0", "stage", lambda x:x[(0,0)].gym_value(), "Stage", 'range_auto'))
    var.append(("Field-0", "Soil-0", "available_Water#L", lambda x:x[(0,0)].value, "Available water (L)", 'range_auto'))
    var.append(("Field-0", "Soil-0", "microlife_health_index#%", lambda x:x[(0,0)].value, "Microlife Soil", 'range_auto'))
    var.append(("Field-0", "Soil-0", "amount_cide#g", lambda x:x['soil'][0,0].value, "Amounct cide (g)", 'range_auto'))
