...
def make_farm(name,field, entities,init_values=None):
    ...
    field1 = Field(localization=field['localization'],
                   shape=field['shape'], entity_managers=entities1)
    ...
    terminal_CNF_conditions = [[(("Field-0", "Weather-0", "day#int365", []), lambda x: x.value, ">=", 360)], [
        (("Field-0", "Plant-0", "global_stage", []), lambda x: x.value, "in", ["dead", "harvested"])]]
    rules = BasicRule(init_configuration=CURRENT_DIR / name_init,
    ...
    farm = Farm(fields=[field1], farmers=[farmer1], scoring=scoring, rules=rules, policies=policies)
    farm.name=name
    return farm
...
def make_monitor(variables):
    ...
    for v in variables:
        ...
        if (v=="plant.cumulated_water#L"):
            var.append(("Field-0", "Plant-0", "cumulated_water#L", lambda x: sum_value(x), "Cumulated water (L)", 'range_auto'))
        ...
        if (v=="plant.cumulated_nutrients_N#g"):
            var.append(("Field-0", "Plant-0", "cumulated_nutrients_N#g", lambda x: sum_value(x), "Cumulated  N (g)", 'range_auto'))
        ...
        if (v=="plant.flowers_per_plant#nb"):
            var.append(
                ("Field-0", "Plant-0", "flowers_per_plant#nb", lambda x: sum_value(x), "Flowers (nb)", 'range_auto'))
        if (v=="plant.pollinator_visits#nb"):
            var.append(
                ("Field-0", "Plant-0", "pollinator_visits#nb", lambda x: sum_value(x), "Pollinator visits (nb)", 'range_auto'))
        if (v=="plant.flowers_pollinated_per_plant#nb"):
            var.append(("Field-0", "Plant-0", "flowers_pollinated_per_plant#nb", lambda x: sum_value(x),
                        "Flowers pollinated (nb)", 'range_auto'))
        ...
        if (v=="plant.fruit_weight#g"):
            var.append(
                ("Field-0", "Plant-0", "fruit_weight#g", lambda x: sum_value(x), "Fruit weight (g)", 'range_auto'))
        if (v=="weeds.grow#nb"):
            var.append(
                ("Field-0", "Weeds-0", "grow#nb", lambda x: sum_value(x), "Weeds grow (nb)", 'range_auto'))
        if (v=="weeds.seeds#nb"):
            var.append(
                ("Field-0", "Weeds-0", "seeds#nb", lambda x: sum_value(x), "Weeds seeds (nb)", 'range_auto'))
        if (v=="weeds.flowers#nb"):
            var.append(
                ("Field-0", "Weeds-0", "flowers#nb", lambda x: sum_value(x), "Weeds flowers (nb)", 'range_auto'))

        if (v=="pests.plot_population#nb"):
            var.append(
                ("Field-0", "Pests-0", "plot_population#nb", lambda x: sum_value(x), "Pest population (nb)", 'range_auto'))
        if (v=="pests.onplant_population#nb.plant"):
            var.append(
                ("Field-0", "Pests-0", "onplant_population#nb", lambda x: sum_value(x["Plant-0"]), "Pest on plant (nb)", 'range_auto'))
        if (v=="pests.onplant_population#nb.weeds"):
            var.append(
                ("Field-0", "Pests-0", "onplant_population#nb", lambda x: sum_value(x["Weeds-0"]), "Pest on weeds (nb)", 'range_auto'))
        if (v=="cides.amount#kg"):
            var.append(
                ("Field-0", "Cide-0", "amount#kg", lambda x: sum_value(x), "Cide Amount (kg)", 'range_auto'))
...
def make_policies_water_harvest(amounts):
    ...
    for amount in amounts:
        ...
        policy_observe= ([[]], [('BasicFarmer-0', 'Field-0', 'Plant-0', 'stage', [(0, 0)])])
        triggered_observations.append(policy_observe)

        triggered_interventions = []
        policy_harvest = ([[(("Field-0", "Plant-0", "stage", [(0, 0)]), lambda x: x, "in", ["ripe"])]], [{'action':('BasicFarmer-0', 'Field-0', 'Plant-0', 'harvest', {}),'delay':0}])
        triggered_interventions.append(policy_harvest)
        policy_harvest = ([[(("Field-0", "Plant-0", "stage", [(0, 0)]), lambda x: x, "in", ["fruit"])]], [{'action':('BasicFarmer-0', 'Field-0', 'Plant-0', 'harvest', {}),'delay':18}])
        triggered_interventions.append(policy_harvest)

        if (amount > 0.):
            policy_water= ([[]], [{'action':('BasicFarmer-0', 'Field-0', 'Soil-0', 'water_continuous', {'plot': (0, 0), 'amount#L': amount, 'duration#min': 60}),'delay':0}])
...
def make_policy_herbicide(amount_herbicide,frequency,amount_water):
        triggered_observations = []
        policy_observe= ([[]], [('BasicFarmer-0', 'Field-0', 'Plant-0', 'stage', [(0, 0)]),('BasicFarmer-0', 'Field-0', 'Weeds-0', 'grow#nb', [(0, 0)])])
        triggered_observations.append(policy_observe)
        ...
        policy_harvest = ([[(("Field-0", "Plant-0", "stage", [(0, 0)]), lambda x: x, "in", ["fruit"])]], [{'action':('BasicFarmer-0', 'Field-0', 'Plant-0', 'harvest', {}),'delay':18}])
        triggered_interventions.append(policy_harvest)
        if (amount_herbicide > 0.):
            policy_herbicide= ([[(("Field-0", 'Weather-0', 'day#int365', []), lambda x: x%frequency, "==", 0)]], [{'action':('BasicFarmer-0', 'Field-0', 'Cide-0', 'scatter', {'plot': (0,0), 'amount#kg':amount_herbicide}),'delay':0}])
        ...
        if (amount_water > 0.):
            policy_water= ([[]], [{'action':('BasicFarmer-0', 'Field-0', 'Soil-0', 'water_continuous', {'plot': (0, 0), 'amount#L': amount_water, 'duration#min': 60}),'delay':0}])
            triggered_interventions.append(policy_water)
        p = policy_API('', triggered_observations, triggered_interventions)
        p.reset()
        return p
...
if __name__ == "__main__":
    ...
    f2 = make_farm("herbtest",
                   {'localization': {'latitude#°': 43, 'longitude#°': 4, 'altitude#m': 150},
                    'shape': {'length#nb': 1, 'width#nb': 1, 'scale#m': 1.}},
                   [(Weather, "dry"), (Soil, "clay"), (Plant, 'corn'), (Pollinators, 'bee'), (Weeds,"base_weed"), (Cide,"herbicide"),(Pests,"basic")],
                   init_values=[('Field-0', 'Weather-0', 'day#int365', 120),
                                ('Field-0', 'Plant-0', 'stage', 'seed'),
                                ('Field-0', 'Soil-0', 'available_N#g', 2500),
                                ('Field-0', 'Soil-0', 'available_P#g', 2500),
                                ...
                                ('Field-0', 'Soil-0', 'available_C#g', 2500)])
    f2.add_monitoring(
        make_monitor(["soil.available_Water#L", "soil.available_N#g", "soil.microlife_health_index#%",
                                                           "plant.pollinator_visits#nb", "plant.size#cm", "plant.flowers_per_plant#nb",
                                                           "plant.flowers_pollinated_per_plant#nb",
                                                           "plant.cumulated_water#L", "plant.cumulated_stress_water#L",
                                                           "plant.cumulated_nutrients_N#g", "plant.cumulated_stress_nutrients_N#g",
                                                           "plant.fruits_per_plant#nb", "plant.fruit_weight#g",
                      "weeds.seeds#nb","weeds.grow#nb","weeds.flowers#nb","cides.amount#kg",
                      "pests.plot_population#nb","pests.onplant_population#nb.plant","pests.onplant_population#nb.weeds"]))
