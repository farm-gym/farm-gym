...
from farmgym.v2.specifications.specification_manager import build_inityaml, build_scoreyaml, build_actionsyaml
...
class Farm(gym.Env):
    def __init__(self, fields, farmers, scoring, rules,policies=None,seed=None):
        ...
        if (self.scoring.score_configuration == None):
            ...
            print(f"[Solution] " + "  Vanilla score configuration file automatically generated in " + str(
                filep + "/" + farm_call + "_score.yaml") + " and used instead.")
        else:
            try:
                ...
                print(f"[Solution] "+" Vanilla score configuration file automatically generated in "+ str(self.scoring.score_configuration) + " and used instead." )

        if (self.rules.init_configuration == None):
            print(f"[Farmgym Warning] Missing initial conditions configuration file.")
            build_inityaml(filep + "/" + farm_call + "_init.yaml", self.fields, mode='default',init_values=self.rules.initial_conditions_values)
            self.rules.init_configuration=filep + "/" + farm_call + "_init.yaml"
            print(f"[Solution] " + " Vanilla initial conditions configuration file automatically generated in " + str(
                filep + "/" + farm_call + "_init.yaml") + " and used instead.")
        else:
            try:
                open(self.rules.init_configuration, 'r', encoding='utf8')
            except FileNotFoundError as err:
                print(f"[Farmgym Warning] Missing initial conditions configuration file.")
                build_inityaml(self.rules.init_configuration, self.fields,mode='default',init_values=self.rules.initial_conditions_values)
                #print('INIT VALUE', self.rules.initial_conditions_values)
                print(f"[Solution] "+"  Vanilla initial conditions configuration file automatically generated in "+ str(self.rules.init_configuration)  + " and used instead.")
        ...
        if (self.rules.actions_configuration == None):
            ...
            print(f"[Solution] "+" Vanilla action configuration file automatically generated in "+ str(filep+ "/"+ farm_call+"_actions.yaml") + " and used instead." )
        else:
            try:
                ...
                print(f"[Solution] "+" Vanilla action configuration file automatically generated in "+ str(self.rules.actions_configuration)  + " and used instead.")
        ...
        self.action_space = MultiUnion(
            [Discrete(1) for i in range(len(self.farmgym_observation_actions))] + [g for fa, fi, e, a,f_a, g in
                                                                                   self.farmgym_intervention_actions],
    ...
    def gym_step(self,gym_action):
        ...
        farmgym_observations, reward, is_done, farmgym_information= self.farmgym_step(self.gymaction_to_farmgymaction(gym_action))
    ...
    def observation_step(self,observation_schedule):
        ...
        for observation_item in observation_schedule:
            ...
            cost = self.scoring.observation_cost(self.farmers[fa_key], self.fields[fi_key], fi_key, entity, variable_key, path)
            #cost = 0
    ...
    def intervention_step(self,action_schedule):
        ...
        return observations, reward, is_done, {"intervention cost": intervention_schedule_cost}
...
#            print("ACTION",action, type(action))
                    ...
                    if (e in action_yaml[fi].keys()):
                       if (action_yaml[fi][e] != None):
                           for action in action_yaml[fi][e]:
                               ...
                               #print(gym_a)
                               actions.append((fa,fi, e,action,action_yaml[fi][e][action],gym_a))
        return actions
                        ...
                        if (action_yaml[fi][e] != None):
                            for var in self.fields[fi].entities[e].variables:
                                if (var in action_yaml[fi][e].keys()):
                                        paths = make(action_yaml[fi][e][var],self.fields[fi].entities[e].variables[var])
    ...
    def render(self, mode='human'):
        ...
        YY = np.max([self.fields[fi].Y +  (int) (np.ceil(len([1 for e in self.fields[fi].entities if self.fields[fi].entities[e].to_thumbnailimage()!=None])/self.fields[fi].X)) for fi in self.fields])
        ...
        font_action = ImageFont.truetype(str(CURRENT_DIR) + "/rendering/Gidole-Regular.ttf", size=im_width*XX//(18*len(self.fields)))
        ...
        dashboard_picture = Image.new('RGBA', (im_width*XX, im_height*YY+offset_header+offset_sep+offset_foot+offset_actions), (255,255,255,255))
        ...
        d.text((dashboard_picture.width // 2 - len(day_string) * font_size//4 , im_height*YY+offset_header+offset_sep+offset_foot//4+offset_actions),day_string ,
               font=font, fill=(100, 100, 100), stroke_width=2,
          stroke_fill="black")
        ...
        for fi in self.fields:
          ...
          #stroke_fill="black")
            d.text((offsetx + (self.fields[fi].X) * im_width // 2 - width_text // 2,
                    offset_header // 4), text, font=font, fill=(100, 100, 100), stroke_width=2,
                   stroke_fill="black")
            ...
            offset_field_y=offset_header+self.fields[fi].Y*im_height+offset_sep+((index-1) // self.fields[fi].X+1)*im_height
            d.rectangle([(offsetx,offset_field_y),(offsetx+self.fields[fi].X*im_width,offset_field_y+offset_actions+im_width//100)], fill=(255,255,255,255), outline=(0,0,0,255), width=im_width//100)
            ...
            if (self.last_farmgym_action):
                for a in self.last_farmgym_action:
                    ...
                    if a[1] == fi and nb_a<= max_display_actions:
                        ...
                        d.text((xx_a,yy_a), text, font=font_action, fill=(100, 100, 100),
                               stroke_width=1,
                               stroke_fill="black")
                        nb_a+=1
...
def generate_video(image_folder='.', video_name='farm.avi'):
    ...
    images = [img for img in os.listdir(image_folder) if ('day-' in img) and (img.endswith(".jpg") or
...
def generate_gif(image_folder='.', video_name='farm.gif'):
    ...
    #os.chdir("/home/ganesh/Desktop/video")
    images = [imageio.imread(img) for img in os.listdir(image_folder) if ('day-' in img) and (img.endswith(".jpg") or
              img.endswith(".jpeg") or img.endswith("png"))]
